...
export class AppComponent {
  ...
  // All ToDo's
  todos$ = this.http.get<ToDo[]>(this.todoUrl).pipe(
  ...
  userName = 'Kamren';
  todosForUser$ = this.http
    .get<User[]>(`${this.userUrl}?username=${this.userName}`)
    .pipe(
      map((users) => users[0]),
      switchMap((user) =>
  ...
  // Uses combineLatest
  dataForUser3$ = this.userSelectedAction$.pipe(
    // Handle the case of no selection
    filter((userName) => Boolean(userName)),
    // Get the user given the user name
    switchMap((userName) =>
      this.http.get<User[]>(`${this.userUrl}?username=${userName}`).pipe(
